...
public class MailSupport {
     ...
     * @return
     */
    public static List<Content> trackStock() {
        ...
        try {
            ...
            for (Message message : messages) {
                ...
                try {
                    ...
                    contentList =  getMailTextContent(mimeMessage, content);
     ...
     * @throws IOException
     */
    public static List<Content> getMailTextContent(Part part, StringBuffer content) throws MessagingException, IOException {
        List<Content> contentList = new ArrayList<>();
        //如果是文本类型的附件，通过getContent方法可以取到文本内容，但这不是我们需要的结果，所以在这里要做判断
        boolean isContainTextAttach = part.getContentType().indexOf("name") > 0;
        if (part.isMimeType("text/*") && !isContainTextAttach) {
            content.append(part.getContent().toString());
            contentList=  parsingTable(part.getContent().toString());
    ...
    private static List<Content>  parsingTable(String table) {
        Document doc = Jsoup.parse(table);
        Elements rows = doc.select("table").get(0).select("tr");
        List<Content> contents = new ArrayList<>();
        Content content = new Content();
        content.setFund("基金");
        content.setDate("操作日期");
...
//                String company = row.select("td").get(6).text();
                ...
                try {
                    content1.setDate(simpleDateFormat.format(format.parse(date)));
...
//                System . out . println ( " % of ETF:" + row . select ( "td" ). get (8 ). text ());
        ...
        contents.forEach(content1 -> {
            ...
            content1.setAmount(amount.toString());
        });
        ...
        contents.stream().filter(a -> a.getPercentage().compareTo("0.1") >= 0)
        ...
        for (Content content1 : contents) {
            ...
            if (content1.getPercentage().compareTo("0.1") >= 0) {
                ...
                String message = content1.getFund() + operate + content1.getTicker() + ": " +
                        ...
                        + "w股" + ",价值" + content1.getAmount() + "w美元,基金仓位占比: " +
        ...
        stringBuilder.append("以上只统计ARK相关基金持仓变化大于0.1%的调仓变化，ARK全部调仓变化如下图");
        //发送邮件
//        try {
